package nam.tran.architechture.widget;
...
import static nam.tran.architechture.R.styleable.*;
import static nam.tran.architechture.R.styleable.Text_fontSize;
import static nam.tran.architechture.R.styleable.Text_fontSpacing;
import static nam.tran.architechture.R.styleable.Text_fontStyle;
import static tran.nam.util.Constant.FONT_STYLES;
import static tran.nam.util.Constant.NON_BREAKING_SPACE;

@SuppressWarnings({"unused", "ConstantConditions"})
public class NTEditText extends AppCompatEditText {

    private static final String TAG = NTEditText.class.getSimpleName();
    ...
    public NTEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        if(!isInEditMode())
            checkCustomAttributes(context, attrs);
    }

    public NTEditText(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void checkCustomAttributes(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            // Hint size
            float fontSizeHint = a.getFloat(Text_fontSizeHint, 0f);
    ...
    private void applyLetterSpacing() {
        ...
        if (this instanceof AppCompatEditText) {
            return;
        }
